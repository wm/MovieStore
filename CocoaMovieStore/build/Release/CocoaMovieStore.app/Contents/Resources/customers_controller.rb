...
class CustomersController < OSX::NSObject
  ...
  ib_outlets :find_customer_text_field, :find_customer_box, :customer_form
  ...
  def create_customer
	...
	begin
	  customer.save
	  puts "Saved Customer with id #{customer.id}"
	rescue
	  puts "failed to save customer"
	  puts customer.to_xml
	end
  end
